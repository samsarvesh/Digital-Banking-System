...
import { apiFetch } from "./services/bankApi";
...
export default function App() {
  ...
  const fetchState = async (keepSelection = true) => {
    setLoading(true);
    try {
      let data: BackendState = await apiFetch("/api/bank/state");
      ...
      if (savedBackupStr) {
        try {
          ...
          if (backup && Array.isArray(backup.accounts) && backup.accounts.length > 0 && data.accounts) {
            ...
            if (needsRestore) {
              ...
              const restoreResult = await apiFetch("/api/bank/restore", {
                method: "POST",
              ...
              if (!restoreResult.error) {
                ...
                const refetchedData = await apiFetch("/api/bank/state");
                if (!refetchedData.error) {
                  data = refetchedData;
  ...
  const fetchCode = async () => {
    try {
      const data = await apiFetch("/api/bank/code");
      if (data.code) {
  ...
  const handleOpenFD = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = await apiFetch("/api/bank/withdraw", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleCloseFD = async (fdId: string) => {
    ...
    try {
      ...
      const data = await apiFetch("/api/bank/deposit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleTransferSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiFetch("/api/bank/transfer", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const executeCloseAccount = async (accNum: number) => {
    ...
    try {
      const data = await apiFetch("/api/bank/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDeleteSelectedTransactions = async () => {
    ...
    try {
      const data = await apiFetch("/api/bank/transactions/delete", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const handleAiQuerySubmit = async (e: React.FormEvent) => {
    ...
    try {
      const data = await apiFetch("/api/bank/ai/advisor", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          prompt: userMessage,
